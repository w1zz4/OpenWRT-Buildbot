...
CMD make FORCE=1 V=s image FILES="files" PACKAGES="collectd collectd-mod-mqtt collectd-mod-memory collectd-mod-interface \
  ...
  odhcpd-ipv6only luci-app-ddns ddns-scripts-cloudflare sqm-scripts luci-app-sqm nlbwmon luci-app-nlbwmon qemu-gam" && \
  mv /home/build/openwrt-imagebuilder-x86-64.Linux-x86_64/bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz /tmp
